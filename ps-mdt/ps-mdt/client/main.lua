...
local tabletProp = `prop_cs_tablet_psmdt`
local tabletBone = 60309
local tabletOffset = vector3(0.03, 0.002, -0.0)
local tabletRot = vector3(10.0, 0.0, 0.0)
...
CreateThread(function()
    if GetResourceState('cd_dispatch') == 'started' then
        TriggerServerEvent("cd-mdt:dispatchStatus", true)
    end
end)
...
RegisterNUICallback("removeCallBlip", function(data, cb)
    TriggerEvent('cd_dispatch:client:removeCallBlip', data.callid or data.id)
    cb(true)
end)
    ...
    AddEventHandler("ps-mdt:client:trafficStop", function()
        ...
        if not coolDown then
            if data.veh ~= nil and data.veh ~= 0 then
                ...
                TriggerServerEvent("cd_dispatch:AddCall", {
                    code = "10-11",
                    title = "Ongoing Traffic Stop",
                    coords = {
                        x = currentPos.x,
                        y = currentPos.y,
                        z = currentPos.z
                    ...
                    details = {
                        description = "Ongoing Traffic Stop at " .. locationInfo,
                        vehicleModel = vehicle.name,
                        plate = lastPlate,
                        officer = plyData.job.grade.name.. ", " ..plyData.charinfo.firstname:sub(1, 1):upper() .. plyData.charinfo.firstname:sub(2) .. " " .. plyData.charinfo.lastname:sub(1, 1):upper() .. plyData.charinfo.lastname:sub(2),
                    },
                    blip = {
                        sprite = 60,
                        color = 3,
