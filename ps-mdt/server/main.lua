...
local ClockinWebhook = 'https://discord.com/api/webhooks/1307378732427903066/aCktkWjAZqko2XN3WxgGrGZIfca_Cb9nzrIOsYF2AR2ZQPo7BHiUb69GlmyhacI11usb'

-- Incident and Incident editting. Add a Discord webhook.
-- Incident Author, Title, and Report will display in webhook post.
local IncidentWebhook = 'https://discord.com/api/webhooks/1307380191131205723/-vgZ3Lut6Qw6cROIyd6k_Cn_MlBOJUMYBeI3eUuBTfoJtyRT3VVAHXcHx2FXghQDDSAz'
--------------------------------
...
AddEventHandler('onResourceStart', function(resourceName)
	...
	if GetResourceState('cd_dispatch') == 'started' then
		local calls = exports['cd_dispatch']:GetDispatchCalls()
		return calls
	end	
end)
...
RegisterNetEvent('mdt:server:openMDT', function()
	...
	if GetResourceState('cd_dispatch') == 'started' then
		calls = exports['cd_dispatch']:GetDispatchCalls()
	end
...
QBCore.Functions.CreateCallback('cd-mdt:getDispatchCalls', function(source, cb)
    if GetResourceState('cd_dispatch') == 'started' then
        local calls = exports['cd_dispatch']:GetDispatchCalls()
        cb(calls)
    else
        cb(nil) -- Handle the case where cd_dispatch is not running
    end
end)
...
RegisterNetEvent('mdt:server:removeMoney', function(citizenId, fine, incidentId)
	...
	if not antiSpam then
		if Player.Functions.RemoveMoney('bank', fine, 'lspd-fine') then
			...
			exports['Renewed-Banking']:addAccountMoney(Officer.PlayerData.job.name, fine)
